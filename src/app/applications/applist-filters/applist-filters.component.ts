...
export class ApplistFiltersComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public gotChanges = false; // future use?
  public loading = false; // to prevent new actions when already loading
  ...
  public ngOnChanges(changes: SimpleChanges) {
    if (changes.applications && !changes.applications.firstChange) {
      ...
      // TODO: should query API instead (see PRC-490)
      // store keys for faster filter lookahead
      // 1) don't include empty results, 2) sort results, 3) remove duplicates
      // this.applicantKeys = _.sortedUniq(_.compact(this.applications.map(app => app && app.client ? app.client.toUpperCase() : null)).sort());
